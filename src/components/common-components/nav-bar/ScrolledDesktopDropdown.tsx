// components/ScrolledDesktopDropdown.tsx
"use client";
...
const ScrolledDesktopDropdown: React.FC<ScrolledDesktopDropdownProps> = ({
  ...
  const buttonStyle = {
    color: isActive || isOpen ? theme.primary : theme.text,
    fontSize: "13px",
    fontWeight: 600,
            ...
            {visibleSubmenus.map((submenu) => {
              ...
              return (
                <Link
                  ...
                  className="flex items-center space-x-3 px-4 py-2 transition-colors duration-200 text-sm"
                  style={{
                    color: submenuIsActive ? theme.primary : theme.text,
                    backgroundColor: submenuIsActive ? `${theme.primary}20` : 'transparent',
                  }}
                  onMouseEnter={(e) => {
                    if (!submenuIsActive) {
                      e.currentTarget.style.color = theme.primary;
                ...
                >
                  {submenu.icon && (
                    <i className={`${submenu.icon} w-3 h-3 text-current`}></i>
                  )}
